...
export type TestSuiteFunc<T, U> = (config: {
	describe(
		title: string,
		func: (func: (title: string, test: (obj: T) => Promise<void>) => Promise<void>) => void,
	): void;
...
export class TestSuite<Fixture, Options extends Record<string, unknown>, FinalFixture> {
	...
	constructor(
		transform: ((fixture: Fixture) => Promise<FinalFixture>) | TestSuiteFunc<FinalFixture, Options>,
	...
	generateTests(options: Options | (() => Promise<Fixture>), fixture?: () => Promise<Fixture>): TestToRun[] {
		...
		function describe(
			title: string,
			func: (func: (title: string, test: (fixture: FinalFixture) => Promise<void>) => Promise<void>) => void,
		): void {
			const subTests: TestToRun[] = [];
			func(async (title: string, test: (fixture: FinalFixture) => Promise<void>) => {
				subTests.push({
					title,
					test: async () => {
		...
		function it(title: string, test: (fixture: FinalFixture) => Promise<void>): void {
			tests.push({
				title,
				test: async () => {
					const obj = await actualFixture();
